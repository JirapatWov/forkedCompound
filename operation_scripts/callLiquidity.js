...
const logBalances = () => {
  return new Promise(async (resolve, reject) => {
    ...
    const liquidityAmountFinal = config.liquidityAmount * Math.pow(10, underlyingDecimals);

    let tx = await underlying.approve(
      cTokenAddress, liquidityAmountFinal.toString()
    );
    await tx.wait(1); // wait until the transaction has 1 confirmation on the blockchain
    const ownerBalance3 = await cToken.liquidateBorrow(config.adminAddress, liquidityAmountFinal.toString(), address.cJPT);

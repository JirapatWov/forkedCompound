...
contract Comptroller is ComptrollerV7Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function borrowAllowed(address cToken, address borrower, uint borrowAmount) override external returns (uint) {
        ...
        if (CustomOracle.getUnderlyingPrice(CToken(cToken)) == 0) {
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        console.log('assets.length : %d', assets.length);
        for (uint i = 0; i < assets.length; i++) {
            ...
            vars.oraclePriceMantissa = CustomOracle.getUnderlyingPrice(asset);
        
            // return (Error.NO_ERROR,vars.oraclePriceMantissa, 0);
            ...
            console.log('vars.oraclePrice : %d', vars.oraclePrice.mantissa);
            console.log('vars.tokensToDenom : %d', vars.tokensToDenom.mantissa);
    ...
    function liquidateCalculateSeizeTokens(address cTokenBorrowed, address cTokenCollateral, uint actualRepayAmount) override external view returns (uint, uint) {
        ...
        uint priceBorrowedMantissa = CustomOracle.getUnderlyingPrice(CToken(cTokenBorrowed));
        uint priceCollateralMantissa = CustomOracle.getUnderlyingPrice(CToken(cTokenCollateral));
    ...
    function _setCollateralFactor(CToken cToken, uint newCollateralFactorMantissa) external returns (uint) {
        ...
        if (newCollateralFactorMantissa != 0 && CustomOracle.getUnderlyingPrice(cToken) == 0) {
